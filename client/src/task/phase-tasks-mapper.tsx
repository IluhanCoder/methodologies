...
function PhaseTasksMapper ({tasks, callBack, assignHandler, isActive = true, deleteHandler, detailsHandler, rights}: LocalParams) {
    ...
    if(tasks[0] && tasks[0].name) return <div className="flex flex-col gap-2">{tasks.map((task: TaskResponse) => task.name && <div className="rounded py-2 pl-10 pr-4 gap-6 shadow flex bg-stone-50 justify-between">
